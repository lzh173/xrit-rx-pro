...
argparser = argparse.ArgumentParser(description="从 HRIT IMG 文件生成 JPEG 图像。")
argparser.add_argument("INPUT", action="store", help="要处理的 HRIT 文件（或文件夹）")
argparser.add_argument("-s", action="store_true", help="将不完整的图像作为单独片段处理")
argparser.add_argument("-o", action="store_true", help="覆盖已存在的图像")
argparser.add_argument("--ext", action="store", help="HRIT 文件扩展名（默认 \".hrit\"）", default=".hrit")
...
def init():
    ...
    if os.path.isdir(args.INPUT):
        ...
        if files.__len__() <= 0:
            print("未找到 \"{}\" IMG 文件".format(args.ext))
            exit(1)
