...
### Arguments ###
argparser = argparse.ArgumentParser(description="GK-2A 红外彩色增强")
argparser.add_argument("INPUT", action="store", help="输入图像路径")
argparser.add_argument("--hot", action="store", help="较热端温度上限，单位开尔文（默认 275K）", default=275)
argparser.add_argument("--cold", action="store", help="较冷端温度下限，单位开尔文（默认 230K）", default=230)
argparser.add_argument("-s", action="store_true", help="禁用 LUT 和文本绘制", default=False)
argparser.add_argument("-o", action="store_true", help="覆盖已存在的增强图像", default=False)
argparser.add_argument("-t", action="store_true", help="仅输出增强区域为透明 PNG", default=False)
