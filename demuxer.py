...
class Demuxer:
    ...
    def __init__(self, config):
        ...
        self.lastImageFD = None         # Last Full Disk image path
        self.lastImageADD = None        # Last Additional Data image path
    ...
    def handle_xRIT(self, spdu):
        ...
        if self.config.images:
            ...
            if self.cProduct.complete:
                ...
                # Track Full Disk and Additional Data images separately
                if self.cProduct.name.mode == "FD":
                    self.demuxer.lastImageFD = self.cProduct.last
                elif self.cProduct.name.type == "ADD" and self.cProduct.name.mode != "ANT":
                    self.demuxer.lastImageADD = self.cProduct.last
                self.cProduct = None
        else:
        ...
        if vcid != self.config.VCID:
            ...
            if self.cTPFile != None:
                ...
                if len(self.cTPFile.PAYLOAD) < self.cTPFile.LENGTH:
                    print("    " + Fore.WHITE + Back.RED + Style.BRIGHT + "文件不完整")
                    ac = len(self.cTPFile.PAYLOAD)
                    ex = self.cTPFile.LENGTH
                    p = round((ac/ex) * 100)
                    print("    " + Fore.WHITE + Back.RED + Style.BRIGHT + "仅达到期望长度的 {}%".format(p))
